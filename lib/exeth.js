...
function Executor() {
	...
	dsl.define('compile', function (cmd, cb) {
		var filename = vals.evaluate(cmd.argumentsText());

		var value = compileContract(filename);
		...
		for (var n in value) {
			...
			if (p >= 0)
				contracts[n.substring(p + 1)] = value[n];
		}
	...
	dsl.define('deploy', function (cmd, cb) {
		...
		txdata.gas = vals.value('gas') || 3000000;
		txdata.gasPrice = vals.value('gasPrice') || 0;
		txdata.data = contracts[contractname].bytecode;
		...
		host.sendTransaction(txdata, function (err, txhash) {
			...
			getTransactionReceipt(txhash, vals.value('timeout') || 60, function (err, txreceipt) {
				...
				vals.value('value', instance.address);
	...
	dsl.define('call', function (cmd, cb) {
		var instancename = cmd.arguments()[0];
		var fnname = cmd.arguments()[1];
		...
		txdata.gas = vals.value('gas') || 3000000;
		txdata.gasPrice = vals.value('gasPrice') || 0;
		txdata.data = contracts[instances[instancename].contract].functionHashes[fnname];
